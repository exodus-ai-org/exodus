/**
 * Depth-aware summarization prompts for LCM.
 *
 * depth 0 → leaf: segment compaction, preserve decisions + rationale
 * depth 1 → D1:  session-level condensation, chronological timeline
 * depth 2 → D2:  phase-level, drop session mechanics, keep outcomes
 * depth 3+ → D3+: strategic/durable context only
 */
